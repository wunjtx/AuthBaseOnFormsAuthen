...
[System.Reflection.Assembly]::LoadFrom((get-item $PSScriptRoot ).parent.FullName+'\bin\AccountManager.WebApp.dll') | Out-null
...
$Connection.ConnectionString=[System.Configuration.ConfigurationManager]::ConnectionStrings["AccountManager"].ConnectionString

$MyPermissionProvider = new-object AccountManager.WebApp.Security.PermissionProvider
...
foreach($permission in $MyPermissions) 
{
    ...
    {
         $sql ="INSERT INTO [Permission](Category,Name,Description,Active) VALUES (@Category,@Name,@Description,@Active)"  
         $Command = New-Object System.Data.SQLClient.SQLCommand
         $Command.Connection = $Connection
		 ...
		 $Command.Parameters.AddWithValue("@Active", $permission.Active) | Out-Null
	...
	{
		 $sql ="UPDATE [Permission] SET Category=@Category,Name=@Name,Description=@Description,Active=@Active WHERE Name=@Name"  
         $Command = New-Object System.Data.SQLClient.SQLCommand
         $Command.Connection = $Connection
         $Command.CommandText = $sql
         $Command.Parameters.AddWithValue("@Category", $permission.Category) | Out-Null
...
foreach($row in $MyDataSet.Tables[0].Rows)
{
    ...
    {
         $sql ="DELETE FROM RolePermission WHERE Permission=@Permission"  
         $Command = New-Object System.Data.SQLClient.SQLCommand
         $Command.Connection = $Connection
         $Command.CommandText = $sql
         $Command.Parameters.AddWithValue("@Permission", $row["Permission"]) | Out-Null
         $Command.ExecuteNonQuery() | Out-Null
